...
def _emulate_with_unicorn_backend(
    ...
    def _shutdown() -> None:
        # Persist any buffered MMIO trace BEFORE tearing the backend down. The
        # recording peripherals only evaluate their flush condition inside an
        # access, so a run that issues fewer than FLUSH_EVERY accesses -- or
        # that stops touching MMIO after early boot -- never writes its tail.
